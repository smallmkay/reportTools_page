...
(function(i){
	...
	i.jqx.dataAdapter.prototype={
					...
					if(W._localdata){
						...
						if(!i.isArray(W.localdata)){
							...
							i.each(W.localdata,function(al){
								...
								if(ad==0){
									if(!(typeof this==="string"||thisinstanceofString)){
										for( var an in this){
											V++;
		...
		loadjson:function(ae,af,R){
			//alert(JSON.stringify(ae)+"======="+JSON.stringify(af)+"========="+JSON.stringify(R))
			if(typeof (ae)=="string"){
				ae=i.parseJSON(ae)
			}
			if(R.root==undefined){
			...
			if(this.virtualmode){
				T=!this.pageable?R.recordstartindex:0;
				P=T;
				T=0;
				J=!this.pageable?this.totalrecords:ae.length;
				//alert(this.pagesize+"===="+J)
			}
			...
			this.records=W;
			//alert("this.records:"+this.records.length)
		...
		tryparsedate:function(ai,aj){
			...
			if(i.type(ai)==="string"){
				...
				if(I){
					...
					var K=ae.parsedate(I,"h:mm tt",aj)||ae.parsedate(I,"h:mm:ss tt",aj)||ae.parsedate(I,"HH:mm:ss.fff",aj)||ae.parsedate(I,"HH:mm:ss.ff",aj)
							||ae.parsedate(I,"HH:mm:ss.tttt",aj)||ae.parsedate(I,"HH:mm:ss",aj)||ae.parsedate(I,"HH:mm",aj)||ae.parsedate(I,"HH",aj);
					var V=0,J=0,Y=0,Z=0;
					if(K&&K.getTime()!=NaN){
						V=K.getHours();
						J=K.getMinutes();
			...
			.extend(
					i.jqx.data,
					{
						...
						ajax:function(M,J){
							...
							J=J||{};
							var P,ad,K,Y,R,V,I,X,Q=i.jqx.data.ajaxSetup({},J),af=Q.context||Q,T=af!==Q&&(af.nodeType||afinstanceofi)?i(af):i.event,ae=i.Deferred(),aa=i.Callbacks("once memory"),N=Q.statusCode
									||{},U={},ab={},L=0,O="canceled",W={
								readyState:0,
	...
	i.jqx.filter=function(){
		...
		var M=0;
		var J=1;
		var P=["EMPTY","NOT_EMPTY","CONTAINS","CONTAINS_CASE_SENSITIVE","DOES_NOT_CONTAIN","DOES_NOT_CONTAIN_CASE_SENSITIVE","STARTS_WITH","STARTS_WITH_CASE_SENSITIVE","ENDS_WITH",
				"ENDS_WITH_CASE_SENSITIVE","EQUAL","EQUAL_CASE_SENSITIVE","NULL","NOT_NULL"];
