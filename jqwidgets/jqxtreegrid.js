...
(function(a){
	...
	a.extend(a.jqx._jqxTreeGrid.prototype,{
		...
		_renderrows:function(){
			...
			var J=N._names();
			//alert("==="+N.source.hierarchy.length)
			//alert("==="+!N.loadingFailed)
			...
			if(D===0){
				...
				var av='<td colspan="'+N.columns.records.length+'" role="gridcell" style="border-right-color: transparent; min-height: '+q+"px; height: "+q+"px;  min-width:"+aq
						+"px; max-width:"+aq+"px; width:"+aq+"px;";
			...
			for( var Q=0;Q<v.length;Q++){
				var at=v[Q];
				//alert("sasde333d"+JSON.stringify(at))
				var ac=at.uid;
				if(m>0){
					if(at[J.level]<m){
						ac=at.uid
				...
				for( var P=0;P<b;P++){
					...
					if((o===1&&!N.rtl)||(M==ab&&N.rtl)||(m>0&&at[J.level]<m)){
						...
						if(this.checkboxes&&!at.aggregate){
							var aC=N.toThemeProperty("jqx-tree-grid-checkbox")+" "+d+" "+N.toThemeProperty("jqx-checkbox-default")+" "+N.toThemeProperty("jqx-fill-state-normal")+" "
									+N.toThemeProperty("jqx-rc-all");
							var g=true;
								...
								if(h){
									...
									if(ax){
										...
										if(ax===false){
											...
											az+="<span class='"+aC+"'><div class='"+N.toThemeProperty("jqx-tree-grid-checkbox-tick")+" "+N.toThemeProperty("jqx-checkbox-check-indeterminate")
													+"'></div></span>"
										}
									}
								}else{
				...
				if(N.rowDetails&&!at.aggregate&&this.rowDetailsRenderer){
					var l='<tr data-role="row-details"><td valign="top" align="left" style="pointer-events: auto; max-width:'+aw+"px; width:"+aw
							+'px; overflow: hidden; border-left: none; border-right: none;" colspan="'+N.columns.records.length+'" role="gridcell"';
					var aI=N.toTP("jqx-cell")+" "+N.toTP("jqx-grid-cell")+" "+N.toTP("jqx-item");
